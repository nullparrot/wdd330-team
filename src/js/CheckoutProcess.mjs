...
const tax_rate = 0.06;
const shipping_first = 8.0;
const shipping_after_first = 2.0;
let subtotal = 0;
let total = 0;
let tax = 0;
let shipping = 0;
...
export function calculate_subtotal() {
  ...
  document.querySelector("#subtotal").innerHTML = `Subtotal: ${subtotal.toFixed(
    2
  )}`;
}
...
export function calculate_the_rest() {
  ...
  document.querySelector("#shipping").innerHTML = `Shipping: ${shipping.toFixed(
    2
  )}`;
  //Calculate Total
  ...
  document.querySelector(
    "#order-total"
  ).innerHTML = `Order Total: ${total.toFixed(2)}`;
}
